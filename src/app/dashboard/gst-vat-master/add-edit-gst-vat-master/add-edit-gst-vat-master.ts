...
@Component({
  selector: 'app-add-edit-gst-vat-master',
   imports: [CommonModule,MaterialModule,RouterModule,ReactiveFormsModule,FormsModule],
  templateUrl: './add-edit-gst-vat-master.html',
  styleUrl: './add-edit-gst-vat-master.scss',
    providers: [TitleCasePipe]
})
export class AddEditGstVatMaster {
  isSaving = false;
...
originalList: any[] = [];
  ...
  private cdr: ChangeDetectorRef,public dialogRef: MatDialogRef<AddEditGstVatMaster>){}
...
updateData(): void {
  ...
  this.http.getAllDataId(ApiUrl.getGstVat,this.categoryId)
    .subscribe({
      next: (res: any) => {
...
onSubmit() {
  this.submit = true;  // <-- mark form as submitted
   ...
   if (
    this.addEditForm.get('rate')?.value === '' ||
    this.addEditForm.get('rate')?.value === null ||
    this.addEditForm.get('rate')?.value === undefined
  ) {
    this.addEditForm.patchValue({
      rate: 0
    });
  ...
  this.http.addEditData(ApiUrl.addEditGstVat, this.addEditForm.value)
    .subscribe({
      next: (res: any) => {
