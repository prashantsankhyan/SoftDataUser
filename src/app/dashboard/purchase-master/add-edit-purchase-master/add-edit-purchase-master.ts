...
import { PurchasePdf } from '../purchase-pdf/purchase-pdf';
import { ConfirmPurchasePItemDelete } from '../confirm-purchase-pitem-delete/confirm-purchase-pitem-delete';



@Component({
  selector: 'app-add-edit-purchase-master',
  ...
  templateUrl: './add-edit-purchase-master.html',
  styleUrl: './add-edit-purchase-master.scss',
   providers: [TitleCasePipe]
})
export class AddEditPurchaseMaster {
...
onKeyDown(event: KeyboardEvent) {
  ...
  if (input.getAttribute('formControlName') === 'rowTotal') {
    ...
    if (!isNaN(rowIndex)) {
      ...
      const row = this.purchaseInvoiceDetails.at(rowIndex) as FormGroup;
...
handleMatSelectEnterIndex(
  ...
  const value = this.purchaseInvoiceDetails.at(i).get('itemId')?.value;
...
onItemKeyDown(
  ...
  const itemControl =
    this.purchaseInvoiceDetails.at(index).get('itemId');
    ...
    const rows =
      Array.from(
        document.querySelectorAll(
          'tbody[formArrayName="purchaseInvoiceDetails"] tr'
        )
      );
...
onTaxDown(event: KeyboardEvent, index: number) {
  const itemControl = this.purchaseInvoiceDetails
    .at(index)
    .get('taxableValueId');
...
onHeadingSelect(event: any) {
 ...
 const headingId = event.value;
   console.log('Selected Heading ID:', headingId);
   this.http.getAllDataId(ApiUrl.getPurchaseHeadingById,headingId)
    .subscribe({
      next: (res: any) => {
...
getPurchaseHeading() {
   return this.http.getAllDataId(ApiUrl.getPurchaaseHeadingByCompanyId, this.companyId)
    .toPromise()
    .then((res: any) => {
...
getAllItemData() {
  ...
  this.filteredItems[index] = [...this.listOfAllItem];
      ...
      this.purchaseInvoiceDetails.controls.forEach(
        (_, index) => {
...
onItemSelect(itemId: number, index: number) {
  ...
  const row =
    this.purchaseInvoiceDetails.at(index);
...
updateData(): void {
  ...
  if (!this.purchaseInvoiceId || this.purchaseInvoiceId <= 0) {
    console.error(
      'Invalid purchaseInvoiceId:',
...
this.isLoading = false;
      ...
      // purchaseInvoiceDetails may come as string after SQL JSON_QUERY
      if (typeof d.purchaseInvoiceDetails === 'string') {
        try {
          d.purchaseInvoiceDetails = JSON.parse(d.purchaseInvoiceDetails);
        } catch (err) {
          console.error('purchaseInvoiceDetails parse failed:', err);
          d.purchaseInvoiceDetails = [];
        }
      }
      ...
      /* ===== PATCH DETAILS ===== */
      this.purchaseInvoiceDetails.clear();
this.itemSearchCtrls = {};
this.filteredItems = {};
      (d.purchaseInvoiceDetails || []).forEach((row: any,index: number) => {

        const fg = this.createPurchaseInvoiceDetail();
        ...
        this.purchaseInvoiceDetails.push(fg);
...
//   this.http.getAllDataIdSale(ApiUrl.getSaleDataForUpdate,this.purchaseInvoiceId)
//     .subscribe((d: any) => {
...
private safeDate(date: string | Date | null): string {
  ...
  getInvoiceHeading() {
    ...
    this.http.getAllDataId(ApiUrl.getPurchaseInvoiceByCompanyId, this.companyId)
      .subscribe((res: any[]) => {
...
isRowInvalid(i: number, control: string, error?: string): boolean {
  const c = this.purchaseInvoiceDetails.at(i).get(control);
  if (!c) return false;
...
onQtyFocus(index: number) {
  const control = this.purchaseInvoiceDetails.at(index).get('qty');
...
clearZeroOnFocus(index: number, controlName: string): void {
  const control = this.purchaseInvoiceDetails.at(index).get(controlName);
...
createPurchaseInvoiceDetail(): FormGroup {
...
addInvoiceDetailRow() {
  const row = this.createPurchaseInvoiceDetail();
  this.purchaseInvoiceDetails.push(row);
  this.listenRowCalculation(row);
   this.recalculateInvoiceTaxRates();
    const index = this.purchaseInvoiceDetails.length - 1;
...
handleAddRow(index: number) {
  ...
  const currentRow = this.purchaseInvoiceDetails.at(index) as FormGroup;
  ...
  // New row index
  const newIndex = this.purchaseInvoiceDetails.length - 1;
  const newRow = this.purchaseInvoiceDetails.at(newIndex) as FormGroup;
...
//   this.purchaseInvoiceDetails.controls.forEach((row: any) => {
...
onRemarksKeyDown(event: KeyboardEvent, index: number): void {
  ...
  const remarksControl = this.purchaseInvoiceDetails
    .at(index)
    .get('remarks');
...
removeInvoiceDetailRow(index: number) {
  ...
  const dialogRef = this.dialog.open(ConfirmPurchasePItemDelete, {
    width: '400px',
    disableClose: true,
    data: {
      title: 'Delete Row',
...
const calculateFromAmount = () => {
  ...
  const index = this.purchaseInvoiceDetails.controls.indexOf(row);
...
recalculateSubTotal() {
  ...
  const rowsTotal = this.purchaseInvoiceDetails.controls.reduce(
    (sum, row: any) => {
...
onRowTotalFocus(index: number): void {
  const control = this.purchaseInvoiceDetails.at(index).get('rowTotal');
...
get totalQty(): string {
  ...
  const total = this.purchaseInvoiceDetails.controls.reduce(
    (sum: number, row: any) => {
...
//   return this.purchaseInvoiceDetails.controls.reduce((sum: number, row: any) => {
...
//     // ✅ FIX QTY HERE
//     purchaseInvoiceDetails: formValue.purchaseInvoiceDetails.map((row: any) => ({
//       ...row,
...
for (let i = 0; i < this.purchaseInvoiceDetails.length; i++) {
  const row = this.purchaseInvoiceDetails.at(i) as FormGroup;
  ...
  this.http.addEditData(ApiUrl.addEditPurchase, payload).subscribe({
    next: (res: any) => {

this.isSaving = false;
       if (res?.purchaseInvoiceId) {
...
this.isSaving = false;
  ...
  this.addEditForm.patchValue({
    purchaseInvoiceId: this.isEditing ? this.purchaseInvoiceId : 0,
    model: 'SaleInvoice',
    companyId: this.companyId,
    shipTo: 0,
    invoiceDate: this.formatDateForInput(new Date()),
...
get isEditMode(): boolean {
  return !!this.purchaseInvoiceId && this.purchaseInvoiceId > 0;
}
...
openInvouveHeadingMaster(data?: any) {
  ...
  dialogRef.afterClosed().subscribe(result => {
    if (result) {
       this.getPurchaseHeading(); // reload list automatically
    }
  });
}
...
openPdfModel() {
  const purchaseInvoiceId =
    this.addEditForm.get('purchaseInvoiceId')?.value;
