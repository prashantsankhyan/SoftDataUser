...
import { AddEditPurchaseMaster } from './add-edit-purchase-master/add-edit-purchase-master';
import { DeletePurchase } from './delete-purchase/delete-purchase';
...
getAllData() {
  ...
  this.api
    .getAllDataId(ApiUrl.getPurchaseInvoiceByCompanyId, this.companyId)
    .subscribe({
      next: (res: any[]) => {
...
addEditData(data?: any) {
  const dialogRef = this.dialog.open(AddEditPurchaseMaster, {
    width: '99vw',
    maxWidth: '2000px',
    maxHeight: '95vh',
...
deleteData(row: any) {
  const dialogRef = this.dialog.open(DeletePurchase, {
    width: '360px',
    disableClose: true,
    data: row.purchaseInvoiceId
  });
