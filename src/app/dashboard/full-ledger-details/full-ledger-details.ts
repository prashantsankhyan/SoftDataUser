...
@Component({
  selector: 'app-full-ledger-details',
  imports: [CommonModule, MaterialModule, ReactiveFormsModule, FormsModule,NgxPrintModule],
  templateUrl: './full-ledger-details.html',
  styleUrl: './full-ledger-details.scss',
})
export class FullLedgerDetails {
...
trackByLedger(index: number, item: any): any {
  return item._id || item.RefId || index;
}

trackByGST(index: number, item: any): any {
...
calculateTotals(data: any[]) {
  ...
  if (this.openingType === 'CR') this.totalCR += opening;
  else this.totalDR += opening;
