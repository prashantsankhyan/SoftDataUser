...
@Component({
  selector: 'app-add-edit-sale-heading',
imports: [CommonModule,MaterialModule,RouterModule,ReactiveFormsModule,FormsModule],
  templateUrl: './add-edit-sale-heading.html',
  styleUrl: './add-edit-sale-heading.scss',
   providers: [TitleCasePipe]
})
export class AddEditSaleHeading {
  isSaving = false;
...
showSpiner = true;
  ...
  private cdr: ChangeDetectorRef,public dialogRef: MatDialogRef<AddEditSaleHeading>){}
...
updateData(): void {
  ...
  this.http.getAllDataId(ApiUrl.getSaleHeading,this.categoryId)
    .subscribe({
      next: (res: any) => {
        ...
        if (res?.success && res.data) {
          ...
          this.addEditForm.patchValue({
            ...
            typeOfSale: data.typeOfSale,
            prefix: data.prefix,
            suffix: data.suffix,
            taxOnSaleType: data.taxOnSaleType,
...
onSubmit() {
  ...
  this.http.addEditData(ApiUrl.addEditSaleHeading, this.addEditForm.value)
    .subscribe({
      next: (res: any) => {
