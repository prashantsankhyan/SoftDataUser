...
@Component({
  selector: 'app-add-edit-heading-master',
   imports: [CommonModule,MaterialModule,RouterModule,ReactiveFormsModule,FormsModule],
  templateUrl: './add-edit-heading-master.html',
  styleUrl: './add-edit-heading-master.scss',
  providers: [TitleCasePipe]
})
export class AddEditHeadingMaster {
  isSaving = false;
...
showSpiner = true;
  ...
  private cdr: ChangeDetectorRef,public dialogRef: MatDialogRef<AddEditHeadingMaster>){}
...
updateData(): void {
  ...
  this.http.getAllDataId(ApiUrl.getDataForEditHeading,this.headingId)
    .subscribe({
      next: (res: any) => {
...
onSubmit() {
  ...
  this.http.addEditData(ApiUrl.addEditHeading, this.addEditForm.value)
    .subscribe({
      next: (res: any) => {
...
get isEditMode(): boolean {
  return !!this.headingId && this.headingId > 0;
}
