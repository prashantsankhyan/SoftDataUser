...
@Component({
  ...
  templateUrl: './delete-heading.html',
  styleUrl: './delete-heading.scss',
})
export class DeleteHeading {
 id ='';
  
  constructor(@Inject(MAT_DIALOG_DATA) public data:any,private http:AllApiService,
  private router:Router,
    private snackBar: MatSnackBar,
   private dialogRef: MatDialogRef<DeleteHeading>) { }
 ...
 deleteRecord() {
  this.http.deleteById(ApiUrl.deleteHeading, this.id).subscribe({
    next: (res: any) => {
      this.snackBar.open(
