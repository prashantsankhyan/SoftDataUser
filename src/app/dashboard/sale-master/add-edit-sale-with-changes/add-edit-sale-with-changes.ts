...
onOpened(type: string, opened: boolean,index?: number) {
  ...
  this.filteredTaxes[index] = this.listOfTaxTableData.filter(
    tax => tax.gstVatClassName === 'Sale/In'
  );
...
canSelectItem(item: any): boolean {
  // Setting ON → allow negative/zero stock
  if (this.screenConfig?.saleOfNegativeStock === true) {
    return true;
  }

  // Setting OFF/null/0 → don't allow zero or negative stock
  return (+item.closingQty || 0) > 0;
}
